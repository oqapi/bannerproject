CREATE TABLE banner (id BIGINT AUTO_INCREMENT, project_id BIGINT NOT NULL, image_url VARCHAR(255), image_text VARCHAR(255), text_font VARCHAR(255), created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, INDEX project_id_idx (project_id), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE banner_position (id BIGINT AUTO_INCREMENT, banner_id BIGINT NOT NULL, position_index VARCHAR(22), x_start BIGINT, y_start BIGINT, x_end BIGINT, y_end BIGINT, INDEX banner_id_idx (banner_id), PRIMARY KEY(id)) ENGINE = INNODB;
