...
<textarea>
<a href="<?php list($width, $height, $type, $attr) = getimagesize(sfConfig::get('sf_root_dir').'/web'.$clientBanner->getUrl($client->sha1ClientText())); echo $clientBanner->getBannerUrl(); ?>?cid=<?php 
$bannertype = is_ani(sfConfig::get('sf_root_dir').'/web'.$clientBanner->getUrl($client->sha1ClientText())); echo $clientBanner->getBannerKind(). '_X_X_'.$bannertype.'_'. $width.'X'.$height.'_'.$client->getClientText();?>" target="_blank"><img src="http://<?php echo $_SERVER['HTTP_HOST'].$clientBanner->getUrl($client->sha1ClientText()); ?>" style="border:none"></a>
</textarea>
